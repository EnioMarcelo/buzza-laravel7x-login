...
@section('content_header')
    ...
    <div class="row">
        <div class="col a-no-color">
            <a href="usuarios">
                <h2 style="margin-left: 10px;"><i class="fa fa-user"></i> Usuários</h2>
            </a>
        </div>

        <div class="col">
            <a class="btn btn-block btn-outline-primary col-md-7 col-lg-7 col-xl-6 m-auto"
               href="{{ route('admin.usuarios.create') }}"><i class="fa fa-plus"></i> Novo Usuário</a>
        </div>
                ...
                @foreach($usuarios as $key => $usuario)
                    ...
                    <tr>
                        ...
                        <td class="text-center">
                            <input type="checkbox" class="btn-active" name="active"
                                   {{ $usuario->active == 1 ? 'checked' : '' }}
                                   data-action="{{ route('admin.usuarios.btnactive',['usuario'=>$usuario->id]) }}"
                                   data-toggle="toggle" data-onstyle="outline-primary" data-offstyle="outline-danger"
                                   data-size="xs" {{ $usuario->id == Auth::user()->id ? 'disabled' : '' }}>
                        </td>
                        <td class="text-center">
                            <div class="text-center">
                                ...
                                <button class="btn badge bg-danger btn_delete_data"
                                        data-action="{{ route('admin.usuarios.destroy',['usuario'=>$usuario->id]) }}"
                                        data-redirect="{{ url()->full() }}" data-toggle="tooltip" data-placement="top"
                                        title="Deletar" {{ $usuario->id == Auth::user()->id ? 'disabled' : '' }}><i
